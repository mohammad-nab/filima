...
class CustomerManager(BaseUserManager):
    def create_user(self, *args, **kwargs):
        ...
        user = self.model(phone_number=kwargs['phone_number'], email=self.normalize_email(kwargs.get('email')) if kwargs.get('email') else None,
    ...
    def create_superuser(self, *args, **kwargs):
        user = self.create_user(phone_number=kwargs['phone_number'], first_name=kwargs['first_name'], last_name=kwargs['last_name'],
                            username=kwargs['username'], password=kwargs['password'], email=kwargs.get('email'))
        user.is_admin = True
        user.save(using=self._db)
        return user